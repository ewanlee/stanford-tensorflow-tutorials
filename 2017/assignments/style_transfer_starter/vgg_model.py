...
def _avgpool(prev_layer, layer_name):
    ...
    """
    with tf.variable_scope(layer_name) as scope:
        pool = tf.nn.avg_pool(prev_layer, ksize=[1, 2, 2, 1],
...
def load_vgg(path, input_image):
    ...
    graph['avgpool1'] = _avgpool(graph['conv1_2'], 'pool1')
    ...
    graph['avgpool2'] = _avgpool(graph['conv2_2'], 'pool2')
    ...
    graph['avgpool3'] = _avgpool(graph['conv3_4'], 'pool3')
    ...
    graph['avgpool4'] = _avgpool(graph['conv4_4'], 'pool4')
    ...
    graph['avgpool5'] = _avgpool(graph['conv5_4'], 'pool5')
