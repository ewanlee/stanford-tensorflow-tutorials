...
w = tf.Variable(tf.random_normal(shape=[784, 10], stddev=0.01), name='weights', dtype=tf.float32)
b = tf.Variable(tf.zeros([1, 10]), name='biases', dtype=tf.float32)
...
with tf.Session() as sess:
    train_writer = tf.summary.FileWriter('./graphs/03/logistic_reg/train', sess.graph)
    test_writer = tf.summary.FileWriter('./graphs/03/logistic_reg/test', sess.graph)
    ...
    for i in range(n_epochs): # train the model n_epochs times
        ...
        for _ in range(n_batches):
            ...
            _, loss_batch, train_summary = sess.run([optimize, loss, merged], feed_dict={X: X_batch, Y: Y_batch})
